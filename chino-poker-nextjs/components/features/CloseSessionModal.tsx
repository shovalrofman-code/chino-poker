  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md rounded-[32px] p-6 border-none shadow-2xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        {players.length === 0 ? (
          <div className="mt-8 mb-4 text-center space-y-6">
            ...
            <div className="space-y-2">
              <div className="text-gray-900 font-black text-lg">אין שחקנים בשולחן</div>
              <p className="text-gray-400 text-xs font-bold leading-relaxed px-4">
                משחק ללא נתונים לא יישמר בהיסטוריה. האם ברצונך לסגור את השולחן מבלי לשמור?
              </p>
            </div>
            <button
              onClick={() => onClose2([])}
              className="w-full casino-btn justify-center py-5 text-base"
            >
              סגור שולחן (לא יישמר)
            </button>
          </div>
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto mt-6 space-y-3 px-1 custom-scrollbar">
              {players.map((p) => (
                <div key={p.playerId} className="flex items-center justify-between bg-white border border-gray-100 p-4 rounded-2xl shadow-sm">
                  <div className="w-28 relative">
                    <input
                      ...
                      className="w-full bg-gray-50 border border-gray-200 rounded-xl ps-8 pe-3 py-2.5 text-center font-black text-sm focus:ring-2 focus:ring-red-100 focus:bg-white focus:border-red-400 focus:outline-none transition-all"
                    />
                    <div className="absolute start-2.5 top-1/2 -translate-y-1/2 text-[10px] font-black text-gray-300 uppercase">אסימונים</div>
                  </div>
                  <div className="text-end">
                    <div className="text-gray-900 font-bold text-sm tracking-tight">{p.firstName} {p.lastName}</div>
                    <div className="text-[10px] text-gray-400 font-bold uppercase tracking-tighter">כניסות: {p.totalBuyins} ₪</div>
                  </div>
                </div>
              ))}
            </div>
